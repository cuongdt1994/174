...
bool Handle_PShopManageFund(PShopManageFund &proto, object_interface &player)
{
	...
	{
		if(player.GetGold() < proto.money) return false;
	...
	else if(proto.optype == 1)//取钱
	{
		...
		if((MAX_CASH_IN_POCKET - player.GetGold()) < money_gain) return false;//金钱满
	}
...
bool Handle_PShopPlayerBuy(PShopPlayerBuy &proto, object_interface &player)
{
	...
	//检查金钱和银票是否够
	if(player.GetGold() < (unsigned int)proto.money_cost) return false;
...
bool Handle_PShopPlayerSell(PShopPlayerSell &proto, object_interface &player)
{
	...
	if((MAX_CASH_IN_POCKET - player.GetGold()) < money_gain)
	{
		unsigned int yp=0;
		unsigned int remain=0;
		MoneyToYinPiao(money_gain, yp, remain);
		if((MAX_CASH_IN_POCKET - player.GetGold()) < remain) return false;
