...
#include "ec_arenateamleave.hpp"
#include "ec_arenateaminviterequest.hpp"
#include "ec_arenateaminvite.hpp"
#include "ec_arenateamkickout.hpp"
#include "ec_arenateamcaptaintransfer.hpp"

#include "ec_arenabattleserverregister.hpp"
#include "ec_arenabattlestart.hpp"
#include "ec_arenabattlestart_re.hpp"
#include "ec_arenaacceptbattle.hpp"
#include "ec_arenaenterqueue.hpp"
#include "ec_arenabattleend.hpp"
...
bool SendSNSRoleBrief(int roleid, object_interface obj_if)
{
	...
	update.brief.level2 	= basic.sec.value;
...
/*	if(domain_type == 0)//C����
	...
	else//A,B����
	{
		...
		{
			if(!GetMnDomainCount(unifid, domain_type - 1))//��B���Ǳ�����C����
				return 3;
		}
	}*/
...
{
	PlayerLuaInfo pli(roleid, gs_id, localsid);
	pli.info.replace(buf, size);
	GProviderClient::DispatchProtocol(0, pli );
	//printf("SendPlayerLuaInfo: roleid = %d \n", roleid);
}

// ec arena 

void EC_GSArenaTeam::GSConvert( EC_SQLArenaTeam * pTeam)
{
	team_id = pTeam->team_id;
	captain_id = pTeam->captain_id;
	team_type = pTeam->team_type;
	score = pTeam->score;
	last_visite = pTeam->last_visite;
	win_count = pTeam->win_count;
	team_score = pTeam->team_score;
	week_battle_count = pTeam->week_battle_count;
	create_time = pTeam->create_time;
	battle_count = pTeam->battle_count;
	
	name_len = pTeam->name.size();
	if (name_len > sizeof(name))
	{
		name_len = sizeof(name);
	}
	memcpy(name, pTeam->name.begin(), name_len );
	
	members.clear();
	for (unsigned int i = 0; i < pTeam->members.size() && i < 6; i++)
	{
		members.push_back({ pTeam->members[i].player_id, pTeam->members[i].award_count});
	}
	
	teams.clear();
	for (unsigned int i = 0; i < pTeam->teams.size() && i < 2; i++)
	{
		teams.push_back( pTeam->teams[i] );
	}
}

void EC_GSArenaTeam::GNETConvert( EC_SQLArenaTeam * pTeam)
{
	pTeam->team_id = team_id;
	pTeam->captain_id = captain_id;
	pTeam->team_type = team_type;
	pTeam->score = score;
	pTeam->last_visite = last_visite;
	pTeam->win_count = win_count;
	pTeam->team_score = team_score;
	pTeam->week_battle_count = week_battle_count;
	pTeam->create_time = create_time;
	pTeam->battle_count = battle_count;
	pTeam->name.replace(name , name_len);
	
	pTeam->members.clear();
	for (unsigned int i = 0; i < members.size() && i < 6; i++)
	{
		pTeam->members.push_back({ members[i].player_id, members[i].award_count});
	}
	
	pTeam->teams.clear();
	for (unsigned int i = 0; i < teams.size() && i < 2; i++)
	{
		pTeam->teams.push_back( teams[i] );
	}
}

void EC_GSArenaPlayer::GSConvert( EC_SQLArenaPlayer * pPlayer)
{
	player_id = pPlayer->player_id;
	team_id = pPlayer->team_id;		
	cls = pPlayer->cls;		
	score = pPlayer->score;	
	win_count = pPlayer->win_count;	
	team_score = pPlayer->team_score;	
	week_battle_count = pPlayer->week_battle_count;
	invite_time = pPlayer->invite_time;		
	last_visite = pPlayer->last_visite;		
	battle_count = pPlayer->battle_count;
	
	name_len = pPlayer->name.size();
	if (name_len > sizeof(name))
	{
		name_len = sizeof(name);
	}
	memcpy(name, pPlayer->name.begin(), name_len );
}

void EC_GSArenaPlayer::GNETConvert( EC_SQLArenaPlayer * pPlayer)
{
	pPlayer->player_id = player_id;
	pPlayer->team_id = team_id;		
	pPlayer->cls = cls;		
	pPlayer->score = score;	
	pPlayer->win_count = win_count;	
	pPlayer->team_score = team_score;	
	pPlayer->week_battle_count = week_battle_count;
	pPlayer->invite_time = invite_time;		
	pPlayer->last_visite = last_visite;		
	pPlayer->battle_count = battle_count;
	pPlayer->name.replace(name , name_len);
}

void EC_SendArenaBattleServerRegister(int world_index, int world_tag)
{
	GProviderClient::DispatchProtocol(0, EC_ArenaBattleServerRegister(world_index, world_tag));
}
...
{
	EC_ArenaTeamCreate proto(ERR_SUCCESS, roleid, cls, team_type, 0,0,0);
	proto.player_name.replace(name, name_size);
	proto.team_name.replace(team_name, team_len);
	GProviderClient::DispatchProtocol(0, proto);
}

void EC_SendArenaTeamInviteRequest(int roleid, int team_type, int invited_roleid, int idx)
{
	GProviderClient::DispatchProtocol(0, EC_ArenaTeamInviteRequest(roleid, team_type, invited_roleid, idx));
}
...
{
	GProviderClient::DispatchProtocol(0, EC_ArenaTeamInvite(idx, team_type, roleid, invited_roleid, invited_cls, Octets(name, name_len)));
}

void EC_SendArenaTeamKickout(int idx, int roleid, int kickid, int team_type)
{
	GProviderClient::DispatchProtocol(0, EC_ArenaTeamKickout(idx, roleid, kickid, team_type));
}

void EC_SendArenaTeamCaptainTransfer(int idx, int roleid, int transferid, int team_type)
{
	GProviderClient::DispatchProtocol(0, EC_ArenaTeamCaptainTransfer(idx, roleid, transferid, team_type));
}

void EC_SendArenaTeamLeave(int idx, int roleid, int team_type)
{
	GProviderClient::DispatchProtocol(0, EC_ArenaTeamLeave(idx, roleid, team_type));
}

// arena queue
void EC_SendArenaEnterQueue(int roleid, int cls, const void * name, unsigned int name_size, int * members, unsigned int count)
{
	EC_ArenaEnterQueue proto(roleid, cls);
	proto.name.replace(name, name_size);
	for (unsigned int i = 0; i < count && i < 6; i++)
	{
		proto.members.push_back(members[i]);
	}
	GProviderClient::DispatchProtocol(0, proto);
}
...
{
	GProviderClient::DispatchProtocol(0, EC_ArenaAcceptBattle(roleid));
}

void EC_SendArenaBattleStart_Re(int arena_id, int world_tag, int retcode)
{
	GProviderClient::DispatchProtocol(0, EC_ArenaBattleStart_Re(retcode, arena_id, world_tag));
}
...
{
	GProviderClient::DispatchProtocol(0, EC_ArenaPlayerTopListQuery(roleid, cmd_mode));
}

void EC_SendArenaTeamTopListQuery(int roleid, int team_type)
{
	GProviderClient::DispatchProtocol(0, EC_ArenaTeamTopListQuery(roleid, team_type));
}

void EC_SendArenaTeamTopListDetailQuery(int roleid, long long team_id)
{
	GProviderClient::DispatchProtocol(0, EC_ArenaTeamTopListDetailQuery(roleid, team_id));
}
...
{
	EC_ArenaBattleEnd proto( arena_id, world_tag, result, red_alive, blue_alive, red_damage, blue_damage );
	
	arena.red_team.GNETConvert( &proto.red_team );
	for (unsigned int i = 0; i < arena.red_members.size() && i < 6; i++)
	{
		EC_SQLArenaPlayer player;
		arena.red_members[i].GNETConvert(&player);
		proto.red_members.push_back(player);
	}
	
	arena.blue_team.GNETConvert( &proto.blue_team );
	for (unsigned int i = 0; i < arena.blue_members.size() && i < 6; i++)
	{
		EC_SQLArenaPlayer player;
		arena.blue_members[i].GNETConvert(&player);
		proto.blue_members.push_back(player);
	}
...
/*
void SendArenaOfAuroraEnd(int arena_id, int world_tag, int result, int red_alive, int blue_alive, int red_damage, int blue_damage)
{
	GProviderClient::DispatchProtocol(0, ArenaBattleEnd(arena_id, world_tag, result, red_alive, blue_alive, red_damage, blue_damage));
}
*/
...
{
	GProviderClient::DispatchProtocol(0,GCodexRequestStorage(roleid,fashion_count,Octets(fashion_data,fashion_size),fly_count,Octets(fly_data,fly_size),
	mount_count,Octets(mount_data,mount_size),pet_count,Octets(pet_data,pet_size)));
}
