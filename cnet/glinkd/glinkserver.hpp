...
struct SessionInfo
{
	...
	SessionInfo() : userid(0), roleid(0), gsid(0), login_time(0), busy(0), local(0), peer(0), checker(NULL), passwd_flag(0), accountinfo_flag(0), market_state(0), load_state(0), hwid(0), homeid(0), teamid(0)
	{
	}
...
class GLinkServer : public TimerManager, public Timer::Observer
{
	...
	int  LoadPolicy(Session::ID sid , Protocol::Type type, unsigned int);
...
public:
	...
	int  GetCountSmile(unsigned char* message, unsigned int size);
...
class AntiDDoSTimer : public Thread::Runnable
{
	int update_time;
public:
	AntiDDoSTimer(int _time,int _proir=1) : Runnable(_proir),update_time(_time) { }
	void Run();
private:
	void L7Timer();
	void OOGTimer();
