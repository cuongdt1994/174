...
{   
	...
	public:
		...
		enum
		{
			...
			MAX_DELAY          = 60,      // 约战最多延迟时间
		};

		typedef std::vector<GTerritoryDetail>  TVector;
		typedef std::map<int,int> ServerMap;
	private:
		time_t BidBeginTime(time_t now) { return t_base + LuaManager::GetInstance()->BidBeginTime(now); }  //BEGIN_BID_TIME;}
		time_t BidDefDuration(time_t now) { return t_base + LuaManager::GetInstance()->BidEndTime(now); }//BEGIN_BID_TIME + DEFAULT_BID_TIME;}
		time_t BidMaxDuration(time_t now) { return t_base + LuaManager::GetInstance()->BidEndTime(now) + MAX_DELAY; }//BEGIN_BID_TIME + DEFAULT_BID_TIME + MAX_DELAY;}
		time_t BattleBeginTime(time_t now) { return t_base + LuaManager::GetInstance()->BattleTime(now); }
		time_t RewardBeginTime(time_t now) { return t_base + LuaManager::GetInstance()->RewardTime(now); }//BEGIN_REWARD_TIME;}
		time_t BattleInterval(size_t num) { return LuaManager::GetInstance()->BattleInterval(num); }
	...
	public:
		...
		bool OnChallenge(short result,int challenge_res,short cid,unsigned int deposit,int fid,int ctime,Protocol& reply);
