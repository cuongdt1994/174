...
public:
	...
	size_t GetTagHwidCounter( int gameid , Octets & hwid )
	{
		size_t counter = 0;
		RoleMap::iterator it = rolemap.begin(), ite = rolemap.end();
		for ( ;it!=ite;++it )
		{
			PlayerInfo* pinfo = it->second;
			if ( pinfo && pinfo->gameid == gameid )
			{
				if ( pinfo->hwid.size() == 8 && hwid.size() == 8 && *(unsigned long long*)pinfo->hwid.begin() == *(unsigned long long*)hwid.begin() )
				{
					counter++;
				}
			}
