...
bool FactionResourceBattleMan::Initialize()
{
    ...
    _clear_time = _end_time + 3600; //ս��������һСʱ��Ϊ����ʱ�䣬һСʱ������ս��
...
bool FactionResourceBattleMan::Update()
{
    ...
    if (_status == STAT_CLOSE) {
        bool is_open_day = _open_day_list[dt.tm_wday]; //���������ж��Ƿ�Ϊ������
        ...
        if ((_second_of_day >= _end_time) && (_second_of_day < _clear_time)) { //�Ƿ���ʱ��
            _status = STAT_BONUS;
            _is_status_changed = 1;
        ...
        if(_player_bonus_list.empty()) {
            if(_second_of_day >= _clear_time) { //���ս����Ϣ
                _status = STAT_CLOSE;
                _is_status_changed = 1;
        ...
        if(_second_of_day >= (_clear_time + TEN_MINUTES)) { //���ս����Ϣ
            _status = STAT_CLOSE;
            _is_status_changed = 1;
            LOG_TRACE("FactionResourceBattle System Change State %d---->%d.\n", STAT_BONUS, STAT_CLOSE);
...
void FactionResourceBattleMan::DoCalcFactionController(CONTROLLER_LIST& list, FactionInfo& info)
{
    ...
    for (std::set<int>::iterator it = domain_id_set.begin(); it != domain_id_set.end(); ++it) {
        if (*it <= 0 || (unsigned int)(*it - 1) >= _controller_list.size()) {
            Log::log(LOG_ERR, "FactionResourceBattle DoCalcFactionController: domain_id=%d out of controller_list range(size=%d), skip.", *it, (int)_controller_list.size());
            continue;
        }
...
void FactionResourceBattleMan::CalcBonus()
{
    ...
    _db_send_bonus_per_sec = _player_bonus_list.size() / 3600 + 1; //Ҫ��60���ӣ���3600���ڽ��������꣬ÿ������һ��
}
