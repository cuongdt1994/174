...
bool CountryBattleMan::Initialize(int gid,bool arrange_country_by_zoneid)
{
	...
	for(unsigned int i = 0; i < open_day_list.size(); ++i) {
		...
		bool flag = false;
		if(npos == 1 && day >=0 && day <= WEEK_DAY_CNT)	// �����ϸ�ʽ
		{
			flag = true;	
		}
		...
		if(flag) 
		{
			...
			if(!zone_list.empty())
			{
				...
				CrossGuardServer::GetInstance()->Register(CT_COUNTRY_BATTLE,day,
					lua->CountryBattleStartTime() - 5*60,
					lua->CountryBattleClearTime()+ 15*60,&zone_list[0],zone_list.size());	
			}
		}
	}
...
float CountryBattleMan::ConvertMajorStrength(int major_str)
{
	...
	switch(major_str)
	{
		case MAJOR_STR_NONE:
			ret = 0.0f;
			break;
		case MAJOR_STR_REFINE16: //ӵ��16Ʒ���������
			ret = 1.0f;
			break;
		case MAJOR_STR_FORCE9: //ӵ��9�����������
			ret = 1.5f;
			break;
		default:
			ret = 0.0f;
...
int CountryBattleMan::ArrangeCountry(bool has_major_str)
{
	int country_id = 0;
	bool is_equal = true;
	size_t _COUNTRY_MAX_CNT = LuaManager::GetInstance()->CountryMaxCount();
	
	int min_str_country = 0;
	if(has_major_str) { //ӵ����Ҫս��
		for(int i = 1; i < _COUNTRY_MAX_CNT; ++i) {
			if( is_equal && (_country_info[i].major_strength != _country_info[i - 1].major_strength) ) is_equal = false;
			
			if(_country_info[i].major_strength < _country_info[min_str_country].major_strength) { //Ѱ����Ҫս����С����Ӫ
				is_equal = false;
				min_str_country = i;
			}
		}
		...
		for(int i = 1; i < _COUNTRY_MAX_CNT; ++i) { 
			...
			if(_country_info[i].minor_strength < _country_info[min_str_country].minor_strength) { //Ѱ�Ҵ�Ҫս����С����Ӫ
				is_equal = false;
				min_str_country = i;
			}
		}
	...
	if(is_equal) { //��Ӫս��ͬ��
		country_id = rand() % _COUNTRY_MAX_CNT + 1; //���һ����Ӫ
	} else {
...
void CountryBattleMan::ArrangeCountryByZoneID()
{
	...
	for(ZONE_COUNTRY_MAP::iterator it = _zone_country_map.begin(); it != _zone_country_map.end(); ++it) {
		...
		int country_id = 0;
		do {
			country_id = rand() % _COUNTRY_MAX_CNT + 1;
...
void CountryBattleMan::PlayerChangeState(PlayerEntry& player, char new_status, int reason)
{
	...
	switch(new_status) {
		...
		{
			ClearMoveInfo(roleid); //�����ҵ��ƶ���Ϣ
			ClearPlayerWaitInfo(roleid); //�����ҵȴ�ս������Ϣ
			break;
		}
...
{
	if(_country_id_ctrl > 0) return _country_id_ctrl; //DEBUG�������ֵ������ʱ��ֱ�ӷ������Ӫ

	bool has_major_str = false;
	for(unsigned int i = 0; i < apply_list.size(); ++i) { //�ж��Ƿ�ӵ����Ҫս��
...
bool CountryBattleMan::PlayerLeaveBattle(int roleid, int country_id)
{
	...
	for(std::vector<int>::iterator it = domain.player_list.begin(); it != domain.player_list.end(); ++it) {
		if(*it == roleid) {
			it = domain.player_list.erase(it); //����������б��У���ҵ�ID
			break;
		}
	}
	
	if(player.status == PLAYER_STATUS_FIGHT) { //��ս����������
	...
	ClearPlayerInfo(roleid); //�����սϵͳ����ҵĸ�����Ϣ
	--_country_info[country_id - 1].online_player_cnt;
	return true;
}
...
void CountryBattleMan::PlayerEnterMap(int roleid, int worldtag)
{
	...
	if(worldtag != _capital_worldtag) { 
		...
		} else { //�ȷǹ�սս�Գ������ֲ���ս��������˵����Ҳ�����ͨ��������NPC;�������˹�ս
			HandlePlayerUnusualSwitchMap(player);
		}
	} else { //�����սս�Գ���
		if(player.status == PLAYER_STATUS_FIGHT) { //��ս�������˳�����սս�Գ���
...
void CountryBattleMan::MoveBetweenDomain(int roleid, int domain_src, int domain_dest)	
{
	...
	while(it != src_domain.player_list.end()) {
		if(*it == roleid) {
			it = src_domain.player_list.erase(it); //����Ҵ�Դ��ص��б���ɾ��
			break;
		}
	...
	dest_domain.player_list.push_back(roleid); //����Ҽ��뵽Ŀ����ص��б���
...
int CountryBattleMan::PlayerMove(int roleid, int dest)
{
	...
	int time_used = GetMoveUseTime(player.in_domain_id, dest); //ȡ���ƶ���Ŀ���������Ҫ��ʱ��
	if(time_used <= 0) {
	...
	DomainInfo& domain = it_domain->second;
	if(domain.owner == player.country_id) { //Ŀ���ͼ�����ͬ��Ӫ
		...
		switch(domain.status) {
			...
			case DOMAIN_STATUS_FIGHT:
			{
				if(domain.challenger == player.country_id) { //�Լ�����Ӫ���ڽ����õ�ͼ
					PlayerMove(roleid, player.in_domain_id, dest, time_used);
					ret = 0;
				}
			...
			case DOMAIN_STATUS_COLD: //Ŀ����ش�����ȴ״̬�������ƶ�
			default:
			{
				break;
			}
...
int CountryBattleMan::PlayerStopMove(int roleid)
{
	...
	PLAYER_ENTRY_MAP::iterator it_player = _player_map.find(roleid);
	if(it_player == _player_map.end()) return -1; //�ڹ�ս���Ҳ��������
		
	PlayerEntry& player = it_player->second;
	if(player.status != PLAYER_STATUS_NORMAL) return -1; //��Ҵ��ڷ�����̬(��ȴ�ս����ս��״̬)�������Խ��ܵ�ȡ���ƶ�������
...
void CountryBattleMan::UpdateMoveInfo()
{
	...
	while(it != _move_info.end()) {
		...
		if(info.time <= 0) { //�Ѿ������˵���Ŀ������Ҫ���ƶ�ʱ��
			PLAYER_ENTRY_MAP::iterator it_player = _player_map.find(info.roleid);
			if(it_player != _player_map.end()) {
				PlayerEntry& player = it_player->second;
				player.prev_domain_id = player.in_domain_id; //���������һ���������
				player.in_domain_id = info.to; //������ҵ�ǰ�������
				...
				if(it_domain != _domain_map.end()) {
					...
					//��ǰ��غ���Ҳ�ͬ��Ӫ�������״̬����Ϊ�ȴ�ս����׼������ս��
					//��ǰ��غ����ͬ��Ӫ�������쵼���ڵȴ�ս����ս��״̬�������֧Ԯ��ս���������״̬����Ϊ�ȴ�ս����׼������ս��
			...
			it = _move_info.erase(it); //��Ϣ������ϣ�����Ҵ��ƶ��б���ɾ��
			continue;
		} else {
			++it;
		}
...
{
	//ս�������NORMAL״̬ת��ΪWAIT_FIGHT״̬����ʵStartBattle����ս��NORMAL״̬�ŵ��õģ������һ�������ж��Է���һ�������ڶദStartBattle	
	if(domain.status != DOMAIN_STATUS_NORMAL) return;
	//׼������ս��
	domain.challenger = challenger; //������ص���ս��ӪID
	DomainChangeState(domain, DOMAIN_STATUS_WAIT_FIGHT, REASON_DOMAIN_WAIT_FIGHT); //���쵼ת��Ϊ�ȴ�ս����״̬

	int player_limit_cnt = LuaManager::GetInstance()->GetConfig()->COUNTRY_PLAYERS_CNT[(int)domain.wartype];
	...
	if(server_id >= 0) {
		//��GS����CountryBattleStartЭ�飬�������ʧ�ܣ�ֱ�ӽ�����ʱ����0�����Ͻ��볬ʱ����
...
void CountryBattleMan::UpdateWaitFightPlayers()
{
	...
	//�ú����ڲ������SysHandlePlayer��������SysHandlePlayer�����ڿ��ܻ�ɾ���ȴ�ս������б����е�Ԫ��
	//�����whileѭ���ڲ��ܵ���SysHandlePlayer����Ҫ�����sys_handle_players�б��ռ�����whileѭ��������һ����
	...
	while(it != _players_wait_fight.end()) {
		...
		if(info.time <= 0) { //�ȴ�ս����״̬��ʱ������ҷ�����Ҫϵͳ���ص��б���һ������
			++it;
		...
		if(it_domain != _domain_map.end()) {
			...
			if(!IsPlayerBeyondBattleLimit(player, domain, 0)) { //������ս���Ļ�����ְҵҪ��ֱ�ӷ���ϵͳ�����б�
				++it;
			...
			switch(domain.status) {
				case DOMAIN_STATUS_NORMAL:
				{
					if(domain.owner != player.country_id) { //����ս��
				...
				case DOMAIN_STATUS_FIGHT:
				{
					if((domain.challenger == player.country_id) || (domain.owner == player.country_id)) { //Ϊ����˫���е�һԱ
						if(!info.is_notify) {
							...
							if(linksid > 0 && proto.localsid > 0) {
								proto.timeout = rand() % 5 + 2; //�ͻ��˳�ʱ��Ӧ��ʱ�������[2, 6]��֮��
...
void CountryBattleMan::UpdateWaitFightDomains(DomainInfo& info)
{
	...
	//�ȴ�GS����ս����ʱ
	//����ֱ�ӱ���player_list����ΪSysHandlePlayer��ı��б����ݣ�������Ҫһ���б��Ŀ���
	...
	info.challenger = 0;
	//��ʱ��Ҫ�������״̬��ת������Ϊ����GSֱ��core���Ŀ��ܣ���������StartBattle_Re����
	//����ֱ�Ӵ�WAIT_FIGHTת��ΪCOLD״̬
...
void CountryBattleMan::UpdateFightDomains(DomainInfo& info)
{
	//���´���ս��״̬������б�
	//����ս��״̬�����GS���и���
	if((info.time + SINGLE_BATTLE_END_TIME + TEN_MINUTES) < Timer::GetTime()) { //����ս���Ѿ�����Ԥ�ڽ���ʱ��10����֮��
		//������˵GS�����ս��������delivery��������״���Ŀ����Էǳ�С
		//�����֣����п���������GS������deliveryû��ͬʱ���������˫��״̬��һ��
		//����ֱ�ӱ���player_list����ΪSysHandlePlayer��ı��б����ݣ�������Ҫһ���б��Ŀ���
		std::vector<int> list = info.player_list;
...
void CountryBattleMan::DBSendBattleBonus(int roleid, int player_bonus, int zoneid)
{
	...
	GRoleInventory inv;
	inv.id = lua->CountryBattleItem();
	inv.count = player_bonus;
	inv.max_count = lua->GetConfig()->COUNTRY_MAX_BONUS;
...
int CountryBattleMan::CalcPlayerBonus(const PlayerScoreInfo& info, int country_bonus[COUNTRY_MAX_CNT])
{
	...
	size_t PLAYER_MAX_BONUS = LuaManager::GetInstance()->GetConfig()->COUNTRY_MAX_BONUS;
...
void CountryBattleMan::OutputZoneCountryScoreLog()
{
	...
	//Initialize���߼�����֤�˿����ս����������Ӧ����[2, COUNTRY_MAX_CNT]֮��
	for(ZONE_COUNTRY_MAP::iterator it = _zone_country_map.begin(); it != _zone_country_map.end(); ++it) {
...
void CountryBattleMan::CalcBonus()
{
	...
	//���Ž���
	int min_cnt_idx = 0;
	for(int i = 1; i < COUNTRY_MAX_CNT; ++i) { //���������������ٵ���Ӫ
	...
	for(int i = 0; i < COUNTRY_MAX_CNT; ++i) { //����������Ӫ���ܷ���
	...
	if(total_scores > 0.0f) {
		for(int i = 0; i < COUNTRY_MAX_CNT; ++i) { //����ÿ����ӪӦ�õĽ�����
			country_bonus[i] += (int)(country_scores[i] / total_scores * (_bonus_limit.total_bonus - base_bonus)); //�ٸ����㷨��ʣ�µĲ��֣������Ƿ�ȫ��
		}
	}

	for(PLAYER_ENTRY_MAP::iterator it = _player_map.begin(); it != _player_map.end(); ++it) { //֪ͨ������ҽ��
	...
	for(PLAYER_SCORE_MAP::iterator it = _player_score_map.begin(); it != _player_score_map.end(); ++it) { //����ÿ�����Ӧ�õõ��Ľ�����
	...
	_db_send_bonus_per_sec = _player_bonus_list.size() / 3000 + 1; //Ҫ��50���ӣ���3000���ڽ��������꣬ÿ������һ��
}
...
{	
	...
	if(_status == ST_CLOSE) {
		bool is_open_day = _open_days[dt.tm_wday]; //���������ж��Ƿ�Ϊ������ //(dt.tm_wday == 0 || dt.tm_wday == 5); //��5�����տ���
		bool is_open_time = (second_of_day >= lua->CountryBattleStartTime()) && (second_of_day < lua->CountryBattleBonusTime());
		...
		if( (second_of_day >= lua->CountryBattleBonusTime()) && (second_of_day < lua->CountryBattleClearTime()) ) { //�Ƿ���ʱ��
			_status = ST_BONUS;
			LOG_TRACE( "CountryBattle System Change State %d---->%d.\n",  ST_OPEN, ST_BONUS);
		...
		if(_player_bonus_list.empty()) {
			if(second_of_day >= lua->CountryBattleClearTime()) { //��չ�ս��Ϣ
				_status = ST_CLOSE;
				LOG_TRACE( "CountryBattle System Change State %d---->%d.\n",  ST_BONUS, ST_CLOSE);
		...
		//������ʱ��Ӧ����22:20��ʼ, 23:30֮ǰ����ȫ�������� ��23��30ʱϵͳ״̬Ӧ�ñ�ΪST_CLOSE
		//���23:40����ST_BONUS��״̬����ʱ˵��ϵͳ�Ѿ����ڲ���ȷ��״̬
		//����ǿ�н�ϵͳת��ΪST_CLOSE��������ݣ�����¼Error Log
		if(second_of_day >= (lua->CountryBattleClearTime() + TEN_MINUTES)) { //��չ�ս��Ϣ
			_status = ST_CLOSE;
			LOG_TRACE( "CountryBattle System Change State %d---->%d.\n",  ST_BONUS, ST_CLOSE);
	...
	_bonus_limit.total_bonus = lua->CountryBattleBonus();
	return true;
}
...
bool CountryBattleMan::RegisterServer(int server_type, int war_type, int server_id, int worldtag)
{
	...
	bool is_find = false;
	for(unsigned int i = 0; i < _servers.size(); ++i) {
		if(server_id == _servers[i].server_id) { //�Ѿ�ע���������ע����Ϣ
...
bool CountryBattleMan::BattleStart(int battleid, int worldtag, int retcode, int defender, int attacker)
{
	...
	//ս��ֻ�д�WAIT_FIGHT״̬����ת��ΪFIGHT״̬��ͬʱ��Ҫ���delivery��GS�Ϲ���˫���Ƿ�һ��
	if(domain.status != DOMAIN_STATUS_WAIT_FIGHT || domain.owner != defender || domain.challenger != attacker) { //״̬����������ݲ�һ��
		if(retcode == 0) { //gsս�������ɹ�
			//֪ͨgs���ٸ�ս��
	...
	const GCountryBattlePersonalScore& score, bool is_winner, float winner_average_score)
{
	...
	if(point > point_ceil) point = point_ceil; //��Ҹ��˵÷ֲ��ܳ�������
	if(!is_winner && (point > winner_average_score)) point = winner_average_score; //ʧ�ܷ����˵÷ֲ��ܳ���ʤ������ƽ����
	...
	const std::vector<GCountryBattlePersonalScore>& defender_score, const std::vector<GCountryBattlePersonalScore>& attacker_score)
{
	...
	int battle_last_time = Timer::GetTime() - domain.time; //����ս��ʱ��
	...
	domain.owner = winner; //�����������Ϊʤ����
	domain.challenger = 0; //�����ս��Ӫ��Ϣ
	
	//ս���������������Ϊ��ȴ״̬
	DomainChangeState(domain, DOMAIN_STATUS_COLD, REASON_DOMAIN_BATTLE_END);
	
	//ȡ�ù���˫���Ƿ�ʹ�ü�Ԯ���mask������������ұ���ս���ķ����ӳ�
...
{
	PLAYER_ENTRY_MAP::iterator it_player = _player_map.find(roleid);
	if(it_player == _player_map.end()) return; //�ڹ�ս���Ҳ��������
	
	PlayerEntry& player = it_player->second;
	int src_domain_id = player.in_domain_id;
	
	if(_status == ST_OPEN) { //��ս״̬�����������ܷ���
		if(player.worldtag != _capital_worldtag) return; //���ֻ�д��ڹ�սս�Գ������ſ��Է����׶�
		if(player.status != PLAYER_STATUS_NORMAL) return; //��Ҵ��ڷ�����̬(��ȴ�ս����ս��״̬)�������Խ��ܵ������׶�������
...
const std::map<int/*linksid*/, PlayerVector>* CountryBattleMan::GetCountryOnlinePlayers(int roleid)
{
	...
	PLAYER_ENTRY_MAP::iterator it_player = _player_map.find(roleid);
	if(it_player == _player_map.end()) return NULL; //�ڹ�ս���Ҳ��������
...
void CountryBattleMan::StopMovingPlayers()
{
	for(MOVE_LIST::iterator it = _move_info.begin(); it != _move_info.end(); ++it) {
		...
		if(it_player == _player_map.end()) continue; //�ڹ�ս���Ҳ��������
...
{
	char mask = domain.battle_config_mask[player.country_id - 1];
	if(!(mask & BATTLE_CONFIG_LIMIT)) return true; //��ز�û����������
...
int CountryBattleMan::GetGroupIdByRoleId(int rid)
{
	...
	if(pinfo && pinfo->user)
	{
		if(GDeliveryServer::GetInstance()->IsCentralDS())
			...
			return CountryBattleMan::_default_group; // �ǵ��� ����Ĭ��0�ŷ���
	}
	return -1;	
}
...
void CountryBattleMan::OnPlayerGetConfig(int rid,unsigned int sid, unsigned int localsid)
{
	...
	if(pinfo && pinfo->user)
	{
		...
		if(!CountryBattleMan::_instance[group].IsActive() && !pinfo->IsGM())		//ֻ�� gm �ܲ�ѯ�ǻ�ڹ�ս��Ϣ
			return;
