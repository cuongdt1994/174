...
{ 
        ...
        class State2:public SkillStub::State
        {
          public:
            ...
            void Calculate(Skill* skill) const
			{
				skill->SetPlus(0.0);
				skill->SetRatio(0.0);
				skill->SetDamage(skill->GetAttack() * (skill->GetLevel() <= 1 ? 1.1 : 1.4));
	...
	{
		return skill->GetPlayer()->GetComboarg()->GetValue(0) == 6060;
	} 
    ...
    bool StateAttack(Skill* skill) const
	{
		...
		skill->GetVictim()->SetValue(skill->GetLevel() <= 3 ? -10.0 : -20.0);
