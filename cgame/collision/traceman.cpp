...
// Static cache: share CSMTree data between world servers loading the same file.
// gs01/gs02 both load world/collision.bht → 1 copy instead of 2.
// arena01-04 all load b01/collision.bht   → 1 copy instead of 4.
// The cache lives for the process lifetime (no cleanup needed at shutdown).
static std::map<std::string, SvrCD::CSMTree*> s_smtree_cache;
...
{
	SvrCD::CSMTree * sm_tree = NULL;
	std::string key(filename);

	std::map<std::string, SvrCD::CSMTree*>::iterator it = s_smtree_cache.find(key);
	if(it != s_smtree_cache.end())
	{
		// Reuse already-loaded tree — saves one full copy of collision data
		sm_tree = it->second;
		printf("共享碰撞树 (已缓存): '%s'\n", filename);
	}
	else
	{
		sm_tree = new SvrCD::CSMTree();
		if(!sm_tree->Load(filename))
		{
			delete sm_tree;
			return false;
		}
		s_smtree_cache[key] = sm_tree;
		printf("加载碰撞树 (首次): '%s'\n", filename);
	}
	...
	brush.sm_ref  = 1;  // cache owns the tree, destructor must not free it
...
bool trace_manager2::LoadPiece(float width, float height, const char * filename)
{
	...
	brush_data brush;
	brush.sm_tree = sm_tree;
	brush.sm_ref  = 1;  // cache owns the tree
	brush.brush_man = new SvrCD::CBrushMan();
...
struct InterSection
{
	...
	struct Pos2D{
		...
		float v;
		bool uaxis; // �ཻ���Ƿ���u���� ��������Ϊv����
		bool uvc;	// �Ƿ���uv������, 0.001f��Χ��
	};
	A3DVECTOR start; // ������Ϊ0,0�������ϵ�е���ʼ��
	A3DVECTOR offset;
	float width;
	float height;
	int max_index;
		...
		: start(sta), offset(off), width(wdt), height(hgt)
	{
		...
		for(int n = start_v; n != end_v; n+=step_v,	cross_point.v += (step_v * height ))
		{
			...
			cross_point_modify.v -= 0.001f; // 0.001f��ֹfloat->intʱ������
		...
		for(int n = start_u; n != end_u; n+=step_u,	cross_point.u += (step_u * width))
		{
			...
			cross_point_modify = cross_point;
			cross_point_modify.u += 0.001f; // 0.001f��ֹfloat->intʱ������
...
bool trace_manager2::AABBTrace(const A3DVECTOR & start, const A3DVECTOR & offset, const A3DVECTOR & ext, bool & in_solid, float &ratio, const abase::vector<char>* element_flags)
{
	...
	{
		A3DVECTOR bc = GetBlockCenter(start_index%_col,start_index/_col,_col,_row, _submap_width, _submap_height); // ���ĵ����ƫ��
		SvrCD::BrushTraceInfo info;
	...
	else
	{
		...
		for(size_t n = 0; n < vt.size(); ++n)
		{
			...
			A3DVECTOR bc = GetBlockCenter(index%_col,index/_col,_col,_row, _submap_width, _submap_height); // ���ĵ����ƫ��
			SvrCD::BrushTraceInfo info;
			info.Init( A3DVECTOR3(start.x - bc.x,start.y + ext.y ,start.z - bc.z), A3DVECTOR3(offset.x,offset.y,offset.z), A3DVECTOR3(ext.x,ext.y,ext.z));
